...
func TestCursorPaging_Select(t *testing.T) {
	db, deferFunc := tests.ConnectToPostgres(t)
	defer deferFunc()

	usersRepo := repo.NewUsersRepo(db)
	cursorPaging := NewCursorPaging(db)
	...
	tests := []struct {
		...
		},
		{
			...
			args: args{
				page: PageableRequest{Size: 1, Cursor: "1"},
			},
			want: Paged[models.User]{
				Content: []models.User{
					{Name: "User #" + strconv.Itoa(2)},
		...
		},
		{
			...
			args: args{
				page: PageableRequest{Size: 2, Cursor: "4"},
			},
			want: Paged[models.User]{
				Content: []models.User{
		...
		},
		{
			...
			args: args{
				page: PageableRequest{Size: 2, Cursor: "6"},
			},
			want: Paged[models.User]{
				Content: []models.User{
	...
	for _, ttt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := cursorPaging.Select(tt.args.ctx, tt.args.page)
			if tt.wantErr {
				assert.Error(t, err)
				return
			}
			...
			assert.Equal(t, tt.want.CursorNextPage, got.CursorNextPage)
...
func TestCursorPaging_SelectAll(t *testing.T) {
	db, deferFunc := tests.ConnectToPostgres(t)
	defer deferFunc()
	...
	for _, ttt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := cursorPaging.SelectAll(tt.args.ctx, tt.args.pageSize)
			assert.NoError(t, err)
